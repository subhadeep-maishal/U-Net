...
with xr.Dataset() as ds_out:
    ...
    # Check the shape of predicted_y and average_actual_nppv
    print(f"Shape of predicted_y: {predicted_y.shape}")
    print(f"Shape of average_actual_nppv: {average_actual_nppv.shape}")

    # Store predicted_nppv and actual_nppv in the dataset with the appropriate dimensions
    ds_out['predicted_nppv'] = (('latitude', 'longitude'), predicted_y[0, :, :])  # Assuming time dimension is not needed for final output
    ds_out['actual_nppv'] = (('latitude', 'longitude'), average_actual_nppv[0, :, :])
